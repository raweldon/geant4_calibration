...
pTSteppingAction::pTSteppingAction(pTEventAction* event):myEvent(event)
{
  ...
  std::string txtfLocation = "/electron_hit_process";
  std::string txtfEnding = ".txt";
  txtFileName = currentDir+txtfLocation + txtfEnding;
...
//          if(origin == "waterphantomPV"){fprintf(txtFile,"%2d ", 1);} else {fprintf(txtFile,"%2d ", 0);}
 //         if(parentTrackID == 1){fprintf(txtFile,"%2d ", 1); std::cout << "parent was a primary" << std::endl;} else {fprintf(txtFile,"%2d ", 0);}
...
//          G4double stepkineticEnergy = aStep->GetPreStepPoint()->GetKineticEnergy(); // gets particle energy "before" collision
          ...
          myEvent->AddgEdep(depositedEnergy);
          G4String dd = "g";
          fprintf(txtFile,"%2s %12.4E %16s %3d %3d\n",dd.c_str(), depositedEnergy, processName.c_str(), parentTrackID, trackID );
                                     }
...
//	  fprintf(txtFile,"%10d ", eID);}  
          ...
          G4String processName = aStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName();
          G4double depositedEnergy1 = aStep->GetDeltaEnergy();
          if (parentTrackID == 1)
          {myEvent->AddEdep(depositedEnergy1);}


          fprintf(txtFile,"%12.4E %16s %3d %3d\n",depositedEnergy1, processName.c_str(), parentTrackID, trackID );
                                 }
